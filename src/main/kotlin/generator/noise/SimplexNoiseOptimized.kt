...
/*
 * Convert to kotlin from java with intelliJ tools
 *
 ...
 *
 */
class SimplexNoiseOptimized(seed: Int) {
    ...
    fun noise(xin: Double, yin: Double, zin: Double): Double {
        ...
        // Skew the input space to determine which simplex cell we're in
        val s =
            (xin + yin + zin) * F3 // Very nice and simple skew factor for 3D
        val i = fastFloor(xin + s)
        val j = fastFloor(yin + s)
    ...
    fun noise(x: Double, y: Double, z: Double, w: Double): Double {
        ...
        val x1 =
            x0 - i1 + G4 // Offsets for second corner in (x,y,z,w) coords
        val y1 = y0 - j1 + G4
        val z1 = z0 - k1 + G4
        val w1 = w0 - l1 + G4
        val x2 =
    ...
    companion object {
        var RANDOMSEED = 0
        private const val NUMBEROFSWAPS = 400
        private val grad3 = arrayOf(
            Grad(1.0, 1.0, 0.0), Grad(-1.0, 1.0, 0.0), Grad(1.0, -1.0, 0.0), Grad(-1.0, -.0, 0.0),
        ...
        private val grad4 = arrayOf(
            ...
            Grad(0.0, -1.0, 1.0, 1.0), Grad(0.0, -1.0, .0, -1.0), Grad(0.0, -1.0, -1.0, 1.0), Grad(0.0, -1.0, -1.0, -1.0),
        ...
        private val p_supply = shortArrayOf(
